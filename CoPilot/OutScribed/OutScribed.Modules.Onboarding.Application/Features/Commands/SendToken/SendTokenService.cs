...
{
    public class SendTokenService(IWriteRepository<TempUser> repository, ITempUserRepository tempUserRepository,
        IEventPublisher publisher, IJailService jail, IEmailEnqueuer email)
    {
        ...
        public async Task<SendTokenResponse> ExecuteAsync(SendTokenRequest request, HttpContext httpContext)
        {
            ...
            // 2. Check Main User Account (critical check)
            // If email already registered in main system, throw appropriate error.

            // 3. Find TempUser by EmailAddress
            //Create Specification
            var existingSpec = new TempUserByEmailSpecification(request.EmailAddress!);
            //Evaluate Specification
            var tempUser = await _repository.FirstOrDefaultAsync(existingSpec); // Check for existence

            if (tempUser == null)
            {
                // New user registration attempt - create new TempUser
                tempUser = TempUser.Create(IdGenerator.Generate(), request.EmailAddress!, ipAddress);
                await _repository.AddAsync(tempUser);
                await _repository.SaveAsync(); // Persist this lock change

                //schedule email
                _email.EnqueueTempUserSendTokenEmail(tempUser.EmailAddress, tempUser.Token.Token);

            }
            else // Existing TempUser found by email (e.g., prior uncompleted attempt)
            {
                ...
                DomainResults result = tempUser.HandleResendRequest(null); // No email update on this path
            ...
            return new SendTokenResponse { Id = tempUser.Id };
        }
    }
}
