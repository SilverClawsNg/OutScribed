...
{
    public class UpdateTaleTagsCommandHandler(IUnitOfWork unitOfWork, IErrorLogger errorLogger)
        : IRequestHandler<UpdateTaleTagsCommand, Result<UpdateTaleTagsResponse>>
    {

        private readonly IUnitOfWork _unitOfWork = unitOfWork;
        private readonly IErrorLogger _errorLogger = errorLogger;

        public async Task<Result<UpdateTaleTagsResponse>> Handle(UpdateTaleTagsCommand request, CancellationToken cancellationToken)
        {
            //Validate request
            var validator = new UpdateTaleTagsCommandValidator();
            ...
            try
            {
                ...
                return new UpdateTaleTagsResponse()
                {
                    Tags = await _unitOfWork.TaleRepository.GetTaleTags(request.Id)
                };
