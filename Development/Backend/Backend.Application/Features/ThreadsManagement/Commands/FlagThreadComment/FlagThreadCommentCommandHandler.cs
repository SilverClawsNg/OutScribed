...
{
    public class FlagThreadCommentCommandHandler(IUnitOfWork unitOfWork, IErrorLogger errorLogger)
        : IRequestHandler<FlagThreadCommentCommand, Result<FlagThreadCommentResponse>>
    {

        private readonly IUnitOfWork _unitOfWork = unitOfWork;
        private readonly IErrorLogger _errorLogger = errorLogger;
       
        public async Task<Result<FlagThreadCommentResponse>> Handle(FlagThreadCommentCommand request, CancellationToken cancellationToken)
        {

            //Validate request
            var validator = new FlagThreadCommentCommandValidator();
            ...
            //save comment
            var result = threads.SaveCommentFlag(request.FlagType, request.UserId, request.CommentId);
            ...
            try
            {
                ...
                //Return success
                return new FlagThreadCommentResponse()
                {
                    Counts = result.Value
                };
