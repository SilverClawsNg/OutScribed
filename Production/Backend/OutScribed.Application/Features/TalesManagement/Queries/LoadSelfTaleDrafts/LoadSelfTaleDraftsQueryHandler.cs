...
{
    public class LoadSelfTaleDraftsQueryHandler(IUnitOfWork unitOfWork)
        : IRequestHandler<LoadSelfTaleDraftsQuery, LoadSelfTaleDraftsQueryResponse>
    {

        private readonly IUnitOfWork _unitOfWork = unitOfWork;

        public async Task<LoadSelfTaleDraftsQueryResponse> Handle(LoadSelfTaleDraftsQuery request, CancellationToken cancellationToken)
        {

            var response = await _unitOfWork.TaleRepository.LoadSelfTaleDrafts(request.UserId, request.Status, request.Category, request.Country, request.Sort, request.Keyword, request.Pointer ?? 0, request.Size ?? 20);
