...
namespace OutScribed.Application.Features.TalesManagement.Commands.UpdateTalePhoto
{
    public class UpdateTalePhotoCommandHandler(IUnitOfWork unitOfWork, 
        IErrorLogger errorLogger, IFileHandler fileHandler)
        : IRequestHandler<UpdateTalePhotoCommand, Result<UpdateTalePhotoResponse>>
    {
        ...
        public async Task<Result<UpdateTalePhotoResponse>> Handle(UpdateTalePhotoCommand request, CancellationToken cancellationToken)
        {
            ...
            string displayPhotoPath = string.Format("outscribed/tales/{0}", displayPhoto);
            ...
            try
            {
                ...
                if (oldPhoto != null)
                {
                    var deleteResult = await _fileHandler.DeleteFileAsync(string.Format("outscribed/tales/{0}", oldPhoto));
