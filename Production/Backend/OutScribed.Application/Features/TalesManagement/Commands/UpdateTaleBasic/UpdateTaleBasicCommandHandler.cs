...
{
    public class UpdateTaleBasicCommandHandler(IUnitOfWork unitOfWork, IErrorLogger errorLogger)
        : IRequestHandler<UpdateTaleBasicCommand, Result<bool>>
    {

        private readonly IUnitOfWork _unitOfWork = unitOfWork;
        private readonly IErrorLogger _errorLogger = errorLogger;

        public async Task<Result<bool>> Handle(UpdateTaleBasicCommand request, CancellationToken cancellationToken)
        {
            //Validate request
            var validator = new UpdateTaleBasicCommandValidator();
            ...
            //Get draft
            Tale? draft = await _unitOfWork.TaleRepository.GetTaleById(request.Id);
            ...
            _unitOfWork.RepositoryFactory<Tale>().Update(draft);
