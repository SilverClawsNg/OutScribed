...
{
    public class FlagThreadCommandHandler(IUnitOfWork unitOfWork, IErrorLogger errorLogger)
        : IRequestHandler<FlagThreadCommand, Result<FlagThreadResponse>>
    {

        private readonly IUnitOfWork _unitOfWork = unitOfWork;
        private readonly IErrorLogger _errorLogger = errorLogger;
       
        public async Task<Result<FlagThreadResponse>> Handle(FlagThreadCommand request, CancellationToken cancellationToken)
        {

            //Validate request
            var validator = new FlagThreadCommandValidator();
            ...
            //save comment
            var result = threads.SaveFlag(request.FlagType, request.UserId);
