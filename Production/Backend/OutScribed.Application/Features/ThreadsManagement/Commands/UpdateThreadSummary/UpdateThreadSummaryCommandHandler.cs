...
{
    public class UpdateThreadSummaryCommandHandler(IUnitOfWork unitOfWork, IErrorLogger errorLogger)
        : IRequestHandler<UpdateThreadSummaryCommand, Result<bool>>
    {

        private readonly IUnitOfWork _unitOfWork = unitOfWork;
        private readonly IErrorLogger _errorLogger = errorLogger;

        public async Task<Result<bool>> Handle(UpdateThreadSummaryCommand request, CancellationToken cancellationToken)
        {
            //Validate request
            var validator = new UpdateThreadSummaryCommandValidator();
            ...
            //upgrade guest
            var result = thread.UpdateSummary(
                request.Summary);
